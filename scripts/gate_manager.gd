...
@export var reward_node_scene: PackedScene
@export var extraction_zone_scene: PackedScene
...
@export var external_reward_amount: int = 24
@export var extraction_countdown: float = 10.0
@export var reward_pickup_radius: float = 2.5
@export var extraction_radius: float = 3.0
@export var reward_offset: Vector3 = Vector3(9.0, 0.0, 5.0)
@export var extraction_offset: Vector3 = Vector3(-9.0, 0.0, 5.0)
...
var _reward_node_collected: bool = false
...
func _process(delta: float) -> void:
	...
	if not _reward_node_collected:
		_try_collect_external_reward()
...
func start_gate_run() -> void:
	...
	status_changed.emit("Gate run started. Defend the drill, collect scrap, then reach extraction.")
	_emit_run_info()
	_broadcast_gate_state()
...
func _try_collect_external_reward() -> void:
	if _reward_node == null:
		return
	for player in players_root.get_children():
		if not player is Node3D:
			continue
		if player.global_position.distance_to(_reward_node.global_position) > reward_pickup_radius:
			continue
		_reward_node_collected = true
		_current_reward += float(external_reward_amount)
		if _reward_node.has_method("set_collected"):
			_reward_node.set_collected(true, external_reward_amount)
		status_changed.emit("Scrap cache secured. Reach extraction when ready.")
		_broadcast_gate_state()
		return


func _try_start_extraction() -> void:
	if _extraction_zone == null:
		return
	for player in players_root.get_children():
		if not player is Node3D:
			continue
		if player.global_position.distance_to(_extraction_zone.global_position) > extraction_radius:
			continue
		_extraction_active = true
		_extraction_remaining = extraction_countdown
		status_changed.emit("Extraction started. Hold out until the countdown completes.")
		_broadcast_gate_state()
		return
...
func _spawn_gate_content_local() -> void:
	...
	if reward_node_scene != null:
		_reward_node = reward_node_scene.instantiate()
		_reward_node.name = "GateRewardNode"
		if _reward_node.has_method("set_collected"):
			_reward_node.set_collected(false, external_reward_amount)
		gate_root.add_child(_reward_node)
		_reward_node.global_position = gate_center + reward_offset

	if extraction_zone_scene != null:
		_extraction_zone = extraction_zone_scene.instantiate()
		_extraction_zone.name = "GateExtractionZone"
		if _extraction_zone.has_method("set_extraction_state"):
			_extraction_zone.set_extraction_state(false, 0.0)
		gate_root.add_child(_extraction_zone)
		_extraction_zone.global_position = gate_center + extraction_offset
...
func _clear_gate_content_local() -> void:
	...
	if _reward_node != null:
		_reward_node.queue_free()
	if _extraction_zone != null:
		_extraction_zone.queue_free()
	_gate_objective = null
	_reward_node = null
	_extraction_zone = null


func _clear_gate_mode(reset_scrap: bool) -> void:
	_gate_active = false
	_extraction_active = false
	_reward_node_collected = false
	_current_reward = 0.0
...
func _emit_run_info() -> void:
	if _gate_active:
		var extraction_text := "Ready" if not _extraction_active else "%0.1fs" % _extraction_remaining
		var reward_text := int(floor(_current_reward))
		run_info_changed.emit("Gate | Scrap %d | Cache %s | Extract %s | Stored %d" % [reward_text, "secured" if _reward_node_collected else "open", extraction_text, _stored_scrap])
		return
	run_info_changed.emit("Base | Stored Scrap %d" % _stored_scrap)


func _broadcast_gate_state() -> void:
	if _reward_node != null and _reward_node.has_method("set_collected"):
		_reward_node.set_collected(_reward_node_collected, external_reward_amount)
	if _extraction_zone != null and _extraction_zone.has_method("set_extraction_state"):
		_extraction_zone.set_extraction_state(_extraction_active, _extraction_remaining)
	_emit_run_info()
	...
	_sync_gate_state.rpc(_gate_active, _current_reward, _reward_node_collected, _extraction_active, _extraction_remaining, _stored_scrap, objective_health, objective_destroyed)
...
func _sync_gate_state(gate_active: bool, current_reward: float, reward_node_collected: bool, extraction_active: bool, extraction_remaining: float, stored_scrap: int, objective_health: float, objective_destroyed: bool) -> void:
	if multiplayer.is_server():
		return
	_gate_active = gate_active
	_current_reward = current_reward
	_reward_node_collected = reward_node_collected
